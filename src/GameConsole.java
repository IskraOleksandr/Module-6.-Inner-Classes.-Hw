...
public class GameConsole implements Powered {
    ...
    public class Gamepad implements Powered {
        ...
        public void powerOn() {
            if (chargeLevel == 0) {
                System.out.println("\nДжойстик номер " + connectedNumber + "  не может быть включен заряд= " + chargeLevel + '%');
            } else {
                isOn = true;
                System.out.println("\nДжойстик номер " + connectedNumber + "  включился заряд= " + chargeLevel + '%');
                if (!getOn()) {
                    setOn(true);
                }
                if (!firstGamepad.isOn()) {
                    secondGamepad.connectedNumber = 1;
                    System.out.println("\nПервый джойстик отключен, второй джойстик становится первым");
                } else {
        ...
        public void powerOff() {
            ...
            System.out.println("\nДжойстик номер " + connectedNumber + "  отключился");
            ;
        }
    ...
    public void playGame() {
        if (isOn) {
            System.out.println("\n Играем в игру: " + activeGame.name);
            if (firstGamepad.isOn) {
                System.out.println("\n   Джойстик номер " + firstGamepad.connectedNumber + " включен и заряжен на " + firstGamepad.chargeLevel + '%');
            }
            if (secondGamepad.isOn) {
                System.out.println("\n   Джойстик номер " + secondGamepad.connectedNumber + " включен и заряжен на " + secondGamepad.chargeLevel + '%');
            }
    ...
    private void changePower(Gamepad gp) {
        if (gp.isOn) {
            ...
            if (gp.getChargeLevel() == 0) {
                gp.isOn = false;
                System.out.println("\nДжойстик номер " + gp.connectedNumber + " полностью разрядился");
            }
        }
    }
    ...
    private void setOn(boolean on) {
        isOn = on;
        System.out.println("\n--Приставка включилась при включении джойстика--");
    }
